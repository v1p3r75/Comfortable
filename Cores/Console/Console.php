...
class Console
{
	...
	private $commands = [
		...
		'serve'=> 'for start the framework in local. You can also set the port width : serve [port]',
	...
	public function run() {
		...
		switch ($this->params[0]) {
			...
			case 'serve':
				...
				$port = $this->params[1] ?? $this->defaultPort;
				print $this->colorText($this->welcomeMsg);
				$cmd = 'php -s -S localhost:' . $port .' -t /';
			...
			case 'migration:run':
                ...
                $migrationName === null ? exit($this->colorText("\nError : You must set the migration name\n",'red')) : null;
				// $runMode = 'up';
				require_once "App/Models/migrations/". $migrationName.".php";
